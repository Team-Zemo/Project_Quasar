    ...
    <div className="flex flex-col items-center w-full max-w-[900px] mx-auto pt-8 pb-16 px-4 md:px-8 overflow-y-auto overflow-x-hidden">
      ...
      <div className="flex items-center gap-3 w-full max-w-[600px] mx-auto mb-12">
        {steps.map((label, i) => {
          ...
          return (
            <div
              key={label}
              className={`flex items-center flex-1 transition-all duration-300 ${isActive ? "opacity-100" : "opacity-40"}`}
            >
              <div className="flex items-center gap-2 w-full">
                <div
                  className={`flex items-center justify-center w-8 h-8 rounded-full text-[13px] font-bold shrink-0 transition-all duration-300 ${
                    isDone
                      ? "bg-[var(--c-accent)] text-white shadow-[0_0_12px_rgba(249,115,22,0.4)]"
                      : isActive
                ...
                <span
                  className={`text-[12px] font-bold whitespace-nowrap transition-colors duration-300 ${isActive ? "text-[var(--c-text)]" : "text-[var(--c-text-mute)]"}`}
                >
                  {label}
                </span>
                {i < steps.length - 1 && (
                  <div
                    className={`h-[2px] w-full ml-2 rounded-full transition-all duration-300 ${isDone ? "bg-[var(--c-accent)]" : "bg-[var(--c-surface-3)]"}`}
                  />
                )}
              </div>
            </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* Step 1: Domain */}
          {step === "domain" && (
            <div className="flex flex-col items-center w-full max-w-[640px] min-h-[68vh] md:min-h-[72vh]">
              <div className="w-full flex flex-col items-center">
                <h1 className="text-[48px] md:text-[56px] font-black tracking-tight text-center text-[var(--c-text)] m-0 mt-6 md:mt-25 leading-[1.1] mb-6 md:mb-4 drop-shadow-sm">
                  Interview{" "}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600 drop-shadow-md">
                    AI
                  </span>
                </h1>
                <p className="text-[16px] md:text-[18px] text-center text-[var(--c-text-dim)] mb-6 md:mb-2 max-w-[80%] leading-relaxed">
                  Practice interviews with a real-time AI interviewer. Select
                  your domain and start talking.
                </p>
              </div>

              <div className="w-full flex flex-col gap-4 mt-2 md:mt-10">
                <div className="relative group w-full flex">
                  ...
                  >
                    ...
                    <ChevronRight
                      size={18}
                      strokeWidth={3}
                      className={domain.trim() ? "translate-x-1" : ""}
                    />
                  </button>
                </div>
              ...
              <div className="w-full flex flex-col gap-3 mt-auto pt-8 md:pt-6">
                <p className="text-[11px] font-bold text-[var(--c-text-dim)] uppercase tracking-wider text-center">
                  Popular Domains
                </p>
                <div className="grid grid-cols-3 gap-2 justify-items-center sm:flex sm:flex-wrap sm:justify-center">
                  {POPULAR_DOMAINS.map((d) => (
                    <button
                      key={d}
                      onClick={() => setDomain(d)}
                      className={`w-full sm:w-auto px-2 sm:px-4 py-2 text-[11px] sm:text-[13px] font-medium rounded-full border transition-all duration-300 text-center whitespace-nowrap overflow-hidden text-ellipsis ${
                        domain === d
