...
function HistogramMetric({
  ...
  const bins = 18;
  const activeBins = Math.max(
    0,
    Math.min(bins, Math.round((value / 100) * bins)),
  );
  ...
  return (
    ...
    >
      ...
      >
        {Array.from({ length: bins }, (_, idx) => (
          <div
            key={`${label}-${idx}`}
            style={{
              height: `${30 + ((idx + 1) / bins) * 70}%`,
              borderRadius: 2,
              background: idx < activeBins ? color : "rgba(255,255,255,0.08)",
...
function HeadPoseCompass({ pitch, yaw }: { pitch: number; yaw: number }) {
  ...
  return (
    <div
      title={`Yaw ${yaw.toFixed(0)}°  Pitch ${pitch.toFixed(0)}°`}
      style={{
        position: "relative",
        width: 36,
        height: 36,
        ...
        border: `1px solid ${on ? "rgba(34,197,94,0.5)" : "rgba(255,255,255,0.12)"}`,
        flexShrink: 0,
      }}
    >
      {/* crosshair */}
...
export function EmotionAnalyzer({
  ...
  const processFrame = useCallback(
    (result: FaceLandmarkerResult, nowMs: number) => {
      ...
      while (
        blinkTimesRef.current.length &&
        blinkTimesRef.current[0] < cutoff
      ) {
      ...
      // Eye contact
      const gazeDev =
        (eyeLookOutL + eyeLookOutR + eyeLookUpL + eyeLookDownL) / 4;
      const facing =
        Math.abs(headPose.yaw) < 20 && Math.abs(headPose.pitch) < 20;
      ...
      const attentionScore = Math.round(
        Math.max(
          0,
          100 -
            Math.min(55, Math.abs(headPose.yaw) * 1.6) -
            Math.min(35, Math.abs(headPose.pitch) * 1.4),
        ),
      );
      ...
      const stressScore = Math.round(
        Math.min(
          100,
          (browInnerUp * 0.25 +
            (browDownL + browDownR) * 0.2 +
            (noseSneerL + noseSneerR) * 0.2 +
            (mouthFrownL + mouthFrownR) * 0.2 +
            cheekPuff * 0.15) *
            120,
        ),
      );
      ...
      if (nervBuf.length >= 4) {
        ...
        const variance =
          nervBuf.reduce((s, v) => s + (v - mean) ** 2, 0) / nervBuf.length;
    ...
    >
      ...
      <div
        style={{
          ...
          borderRadius: 16,
          overflow: "hidden",
          border: `2px solid ${eyeContact ? "rgba(249,115,22,0.35)" : "rgba(255,255,255,0.08)"}`,
          transition: "border-color 0.5s ease",
          boxShadow: eyeContact
            ? "0 0 16px rgba(249,115,22,0.12) inset"
            : "none",
        }}
      >
        <video
        ...
        >
          <div
            style={{
              ...
              height: 7,
              borderRadius: "50%",
              background: analyzing && faceOk ? "#ef4444" : "#6b7280",
              boxShadow:
                analyzing && faceOk ? "0 0 6px rgba(239,68,68,0.8)" : "none",
              animation:
                analyzing && faceOk
                  ? "ea-pulse 1.5s ease-in-out infinite"
                  : "none",
            }}
          />
          <span
        ...
        >
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 17, fontWeight: 900, color: "#d1d5db" }}>
                {attention}%
              </span>
            </div>
            ...
            >
              ...
              <span style={{ fontSize: 17, fontWeight: 900, color: "#f87171" }}>
                {stress}%
              </span>
            </div>
          </div>
      ...
      {analyzing && (
        ...
        >
          ...
          >
            ...
            >
              P{pose.pitch >= 0 ? "+" : ""}
              {pose.pitch.toFixed(0)}°&nbsp; Y{pose.yaw >= 0 ? "+" : ""}
              {pose.yaw.toFixed(0)}°&nbsp; R{pose.roll >= 0 ? "+" : ""}
              {pose.roll.toFixed(0)}°
            </span>
          </div>
        </div>
      ...
      {analyzing && faceOk && (
        ...
        >
          ...
          <HistogramMetric
            ...
            color="#9ca3af"
          />
          <HistogramMetric label="Stress" value={stress} color="#f87171" />
        </div>
      )}
