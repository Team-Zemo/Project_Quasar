...
export function ProfilePage() {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setMessage({
          type: "error",
          text: json.message || "Failed to upload resume",
        });
      }
    } catch {
  ...
  const handleSyncPlatforms = async () => {
    ...
    try {
      ...
      if (json.success) {
        setProfile((prev) =>
          prev ? { ...prev, platformSyncStatus: "syncing" } : prev,
        );
        setMessage({
          type: "success",
          text: "Platform sync started in background",
        });
      } else {
        setMessage({
          type: "error",
          text: json.message || "Failed to start sync",
        });
      }
    } catch {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 auto-rows-min">
        ...
        <div className="xl:col-span-4 xl:row-span-2 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] p-6 xl:min-h-[280px]">
          <div className="flex flex-col items-center text-center gap-4">
            <IdenticonAvatar
              ...
              size={88}
              className="rounded-full flex-shrink-0 shadow-lg ring-2 ring-[var(--c-border)]"
            />
            <div className="w-full">
              {editing ? (
                <input
                  ...
                  className="text-[28px] text-center font-black text-[var(--c-text)] bg-[var(--c-bg)] border border-[var(--c-border)] rounded-xl px-3 py-1.5 w-full mb-1 outline-none focus:border-[var(--c-accent)]"
                />
              ) : (
                <h1 className="text-[30px] leading-[1.05] font-black tracking-tight text-[var(--c-text)] mb-1">
                  {profile.name}
                </h1>
              )}
              {isCandidate &&
                (editing ? (
                  <input
                    ...
                    className="text-[15px] text-center text-[var(--c-text-dim)] bg-[var(--c-bg)] border border-[var(--c-border)] rounded-xl px-3 py-1.5 w-full outline-none focus:border-[var(--c-accent)]"
                  />
                ) : (
                  <p className="text-[16px] text-[var(--c-text-dim)]">
              ...
              <div className="flex flex-col items-center gap-y-1 mt-3 text-[14px] text-[var(--c-text-mute)]">
                ...
                {isCandidate && profile.experience != null && (
                  <span className="flex items-center gap-1">
                    <Briefcase size={14} /> {profile.experience} yrs exp
                  </span>
                )}
              </div>
            </div>
              ...
              {editing ? (
                <>
                  <button
                    ...
                    className="p-2 rounded-xl hover:bg-[var(--c-surface-2)] text-[var(--c-text-mute)]"
                    title="Cancel"
                  >
                    <X size={18} />
                  </button>
                  <button
                    ...
                    disabled={saving}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-bold bg-[var(--c-accent)] text-white hover:brightness-110 transition-all"
                  >
                    {saving ? (
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-bold border border-[var(--c-border)] text-[var(--c-text-dim)] hover:bg-[var(--c-surface-2)] transition-all"
                >
        ...
        {/* Personal Info Card */}
        <div className="xl:col-span-8 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] p-6">
          <h2 className="text-[22px] font-black text-[var(--c-text)] mb-5 flex items-center gap-2">
            <UserIcon size={20} className="text-[var(--c-accent)]" />
            ...
            {isRecruiter && (
              <>
                ...
                <EditableField
                  ...
                  editing={editing}
                  onChange={(v) =>
                    setForm((prev) => ({ ...prev, companyWebsite: v }))
                  }
                  icon={Globe}
                  placeholder="https://..."
                />
        ...
        {isCandidate && (
          <div className="xl:col-span-4 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] p-6">
            <h2 className="text-[22px] font-black text-[var(--c-text)] mb-5">
              Skills
            </h2>
            <div className="flex flex-wrap gap-2 mb-3">
              {form.skills.length === 0 && !editing && (
                <p className="text-[14px] text-[var(--c-text-mute)] italic">
                  No skills added yet. Upload a resume to auto-fill.
                </p>
              )}
              {form.skills.map((s) => (
                <span
                  key={s}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[13px] font-semibold bg-[var(--c-accent-dim)] text-[var(--c-accent)] border border-[var(--c-accent)]/20"
                >
                  {s}
                  {editing && (
                    <button
            ...
            {editing && (
              <div className="flex gap-2">
                ...
                <button
                  onClick={addSkill}
                  className="p-2 rounded-xl bg-[var(--c-accent-dim)] text-[var(--c-accent)] hover:bg-[var(--c-accent)] hover:text-white transition-all"
                >
                  <Plus size={16} />
                </button>
              </div>
        ...
        {isCandidate && (
          <div className="xl:col-span-4 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[22px] font-black text-[var(--c-text)] flex items-center gap-2">
                <Globe size={20} className="text-[var(--c-accent)]" />
                External Platforms
              </h2>
              {profile.platformSyncStatus && (
                <span
                  className={`text-[13px] font-semibold px-2.5 py-1 rounded-lg ${
                    profile.platformSyncStatus === "completed"
                      ? "bg-[var(--c-success-dim)] text-[var(--c-success)]"
                      : profile.platformSyncStatus === "syncing"
                        ? "bg-blue-500/10 text-blue-500"
                        : profile.platformSyncStatus === "failed_fetching"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <EditableField
                ...
                editing={editing}
                onChange={(v) => setForm((prev) => ({ ...prev, githubUrl: v }))}
                icon={Globe}
              ...
              <EditableField
                ...
                editing={editing}
                onChange={(v) =>
                  setForm((prev) => ({ ...prev, leetcodeUrl: v }))
                }
                icon={Globe}
            ...
            <div className="mt-4 pt-4 border-t border-[var(--c-border)] flex items-center justify-between">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-bold text-[var(--c-text-mute)] hover:text-[var(--c-text)] hover:bg-[var(--c-surface-2)] transition-all border border-[var(--c-border)]"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-bold bg-[var(--c-accent-dim)] text-[var(--c-accent)] hover:bg-[var(--c-accent)] hover:text-white transition-all"
              >
                {syncingPlatform ? (
          ...
          <div className="xl:col-span-8 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] p-6">
            ...
            {profile.resumeKey ? (
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between bg-[var(--c-bg)] rounded-xl p-4 border border-[var(--c-border)]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[13px] text-[var(--c-text-mute)]">
                        Uploaded{" "}
                  ...
                  <div className="flex gap-2">
                    <label className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-bold border border-[var(--c-border)] text-[var(--c-text-dim)] hover:bg-[var(--c-surface-2)] transition-all cursor-pointer">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl text-[var(--c-text-mute)] hover:text-[var(--c-error)] hover:bg-[var(--c-error-dim)] transition-all"
                      title="Delete resume"
                    >
                      <Trash2 size={14} />
                    </button>
                ...
                {uploadingResume && (
                  <div className="flex items-center gap-2 text-[14px] text-[var(--c-accent)]">
                    <Loader2 size={17} className="animate-spin" /> Uploading and
                    parsing...
                  </div>
                )}
              </div>
              ...
              <div className="border-2 border-dashed border-[var(--c-border)] rounded-2xl p-8 text-center hover:border-[var(--c-accent)]/40 transition-colors">
                <Upload
                  size={32}
                  className="mx-auto text-[var(--c-text-mute)] mb-3"
                />
                <p className="text-[18px] font-semibold text-[var(--c-text)] mb-1">
                  Upload your resume
                </p>
                <p className="text-[14px] text-[var(--c-text-mute)] mb-4">
                  PDF format, max 10 MB. AI will auto-extract your skills and
                  experience.
                </p>
                <label className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-[14px] font-bold bg-[var(--c-accent)] text-white hover:brightness-110 transition-all cursor-pointer">
                  {uploadingResume ? (
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[var(--c-border)]">
            <span className="text-[13px] font-semibold uppercase tracking-wider text-[var(--c-text-mute)] w-[120px]">
              Linked
            </span>
            <div className="flex gap-2">
              {hasGoogle && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[12px] font-semibold bg-[var(--c-surface-2)] border border-[var(--c-border)] text-[var(--c-text)]">
              ...
              {!hasGoogle && !hasGitHub && (
                <span className="text-[12px] text-[var(--c-text-mute)]">
                  No linked accounts
                </span>
              )}
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl text-[13px] font-bold border border-[var(--c-border)] text-[var(--c-text-dim)] hover:bg-[var(--c-surface-2)] transition-all"
            >
              {showPasswordForm
                ? "Cancel"
                : hasPassword
          ...
          {showPasswordForm && (
            <form
              ...
              className="mt-4 pt-4 border-t border-[var(--c-border)] space-y-3"
            >
              {hasPassword && (
                <input
                  type="password"
                  ...
                  className="w-full bg-[var(--c-bg)] border border-[var(--c-border)] rounded-xl px-3 py-2.5 text-[14px] text-[var(--c-text)] outline-none focus:border-[var(--c-accent)]"
                />
              )}
              <input
                type="password"
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[14px] font-bold bg-[var(--c-text)] text-[var(--c-bg)] hover:brightness-110 transition-all"
              >
                {passwordLoading ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : null}
