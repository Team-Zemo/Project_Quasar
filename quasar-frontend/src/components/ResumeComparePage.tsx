      ...
      {file ? (
        <div className="flex flex-col items-center text-center w-full z-10 transition-all">
          ...
          <div className="flex flex-col max-w-full px-4 gap-1">
            <span className="text-[14px] font-semibold text-[var(--c-text)] truncate w-full">
              {file.name}
            </span>
            <span className="text-[12px] font-medium text-[var(--c-text-dim)]">
        ...
        <div className="flex flex-col items-center text-center z-10">
          ...
          <span className="text-[15px] font-bold text-[var(--c-text)] mb-1.5">
            {label}
          </span>
          <span className="text-[13px] text-[var(--c-text-mute)]">
            Click or drag & drop · PDF · Max 10 MB
          </span>
        </div>
      )}
    </div>
...
export function ResumeComparePage() {
  ...
  const handleCompare = async () => {
    ...
    if (jdMode === "text" && jdText.trim().length < 50) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto px-4 py-8 md:py-12 min-h-[calc(100vh-64px)]">
      {/* ── Header ── */}
      <div className="flex flex-col items-center text-center md:mt-20 max-w-2xl mb-12">
        ...
        <p className="text-[16px] leading-[1.6] text-[var(--c-text-dim)] m-0 [text-wrap:balance]">
          Upload your resume and a job description — get an instant AI-powered
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 md:gap-8 w-full items-stretch">
        ...
        <div className="flex flex-col bg-[var(--c-surface)] border border-[var(--c-border)] rounded-3xl p-6 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between mb-5 select-none">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[16px] font-bold text-[var(--c-text)]">
                Your Resume
              </span>
            </div>
            <span className="text-[12px] font-bold uppercase tracking-wider text-[var(--c-text-mute)] bg-[var(--c-surface-2)] px-2 py-0.5 rounded-md">
              PDF only
            </span>
          </div>
          <DropZone
        ...
        <div className="flex flex-row md:flex-col items-center justify-center gap-4 py-4 md:py-0">
          ...
          <span className="flex flex-col items-center justify-center w-10 h-10 rounded-full bg-[var(--c-surface-2)] border border-[var(--c-border)] text-[12px] font-black text-[var(--c-text-mute)] shrink-0 z-10">
            VS
          </span>
          <div className="flex-1 w-px md:h-full bg-gradient-to-b from-transparent via-[var(--c-border-2)] to-transparent max-md:h-px max-md:w-full" />
        </div>
      ...
      {error && (
        <div
          className="flex items-center gap-3 w-full bg-red-500/10 border border-red-500/20 text-red-500 px-4 py-3 rounded-2xl mt-8 text-[14px] font-medium"
          role="alert"
        >
          <svg
            width="16"
      ...
      <button
        ...
        disabled={
          loading ||
          !resumeFile ||
          (jdMode === "pdf" ? !jdFile : jdText.trim().length < 50)
        }
      >
        {loading ? (
          <>
      ...
      {result && (
        <div
          className="w-full flex flex-col gap-8 mt-12 animate-in fade-in slide-in-from-bottom-4 duration-500"
          id="compare-results"
        >
          {/* Score hero */}
          <div className="flex flex-col md:flex-row items-center gap-8 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-3xl p-6 md:p-10 shadow-sm relative overflow-hidden">
            ...
            <div className="flex flex-col flex-1 items-center md:items-start text-center md:text-left z-10 w-full">
              ...
              <p className="text-[15px] leading-relaxed text-[var(--c-text-dim)] mb-6 max-w-2xl">
                {result.summary}
              </p>
          ...
          {/* Tab nav */}
          <div
            className="flex overflow-x-auto gap-2 p-1.5 bg-[var(--c-surface-2)] border border-[var(--c-border)] rounded-2xl custom-scrollbar"
            role="tablist"
          >
            ...
            <div className="flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                ...
                <div className="flex flex-col bg-[var(--c-surface)] border border-[var(--c-border)] rounded-2xl p-6 shadow-sm overflow-hidden relative">
                  ...
                  <div className="flex justify-between items-baseline mb-2 pb-2 border-b border-[var(--c-border-2)] text-[14px]">
                    <span className="text-[var(--c-text-mute)]">Required</span>
                    <strong className="text-[var(--c-text)] truncate max-w-[60%] text-right">
                      {result.educationAnalysis.required}
                    </strong>
                  </div>
                  <div className="flex justify-between items-baseline mb-4 text-[14px]">
                    <span className="text-[var(--c-text-mute)]">Candidate</span>
                    <strong className="text-[var(--c-text)] truncate max-w-[60%] text-right">
                      {result.educationAnalysis.candidate}
                    </strong>
                  </div>
                  <div className="mt-auto pt-3 border-t border-[#f97316]/20 text-[13px] font-medium text-[#f97316] leading-snug">
              ...
              <div className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-2xl p-6 shadow-sm">
                ...
                <ul className="flex flex-col gap-3 m-0 p-0 list-none">
                  {result.keyStrengths.map((s, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 p-3.5 bg-[var(--c-success-dim)] border border-[var(--c-success)]/10 rounded-xl text-[14px] leading-relaxed text-[var(--c-text)]"
                    >
                      <svg
                        width="16"
                        height="16"
              ...
              <div className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-2xl p-6 shadow-sm">
                ...
                <ul className="flex flex-col gap-3 m-0 p-0 list-none">
                  {result.improvementAreas.map((s, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 p-3.5 bg-orange-500/5 border border-orange-500/10 rounded-xl text-[14px] leading-relaxed text-[var(--c-text)]"
                    >
                      <svg
                        width="16"
                        height="16"
          ...
          {activeTab === "skills" && (
            <div className="flex flex-col animate-in fade-in zoom-in-95 duration-300">
              {result.matchedSkills.length === 0 ? (
                <p className="text-center py-24 text-[var(--c-text-mute)] text-[15px] bg-[var(--c-surface-2)] border border-[var(--c-border)] border-dashed rounded-3xl m-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.matchedSkills.map((sk, i) => (
                    <div
                      key={i}
                      className="flex flex-col bg-[var(--c-surface)] border border-[var(--c-success)]/20 shadow-[inset_0_4px_24px_rgba(34,197,94,0.03)] rounded-2xl p-5 mb-0"
                    >
                      <div className="flex items-center justify-between mb-3 border-b border-[var(--c-border-2)] pb-3">
                        <span className="text-[15px] font-bold text-[var(--c-text)] truncate max-w-[50%]">
                          {sk.skill}
                        </span>
                        <div className="flex items-center gap-2">
                          <ProficiencyDots level={sk.proficiency} />
                          <span className="text-[12px] font-semibold text-[var(--c-text-dim)] capitalize">
                            {sk.proficiency}
                          </span>
                        </div>
                      </div>
                      <p className="text-[13px] leading-relaxed text-[var(--c-text-dim)] m-0">
                        {sk.context}
                      </p>
                    </div>
                  ))}
                ...
                <div className="mt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {result.bonusSkills.map((sk, i) => (
                      <div
                        key={i}
                        className="flex flex-col bg-purple-500/5 border border-purple-500/20 rounded-2xl p-5"
                      >
                        <span className="text-[15px] font-bold text-[#a855f7] mb-2">
                          {sk.skill}
                        </span>
                        <p className="text-[13px] leading-relaxed text-[var(--c-text-dim)] m-0">
                          {sk.relevance}
          ...
          {activeTab === "gaps" && (
            <div className="flex flex-col animate-in fade-in zoom-in-95 duration-300">
              {result.missingSkills.length === 0 ? (
                <p className="text-center py-24 text-[var(--c-success)] font-bold text-[16px] bg-[var(--c-success-dim)] border border-[var(--c-success)]/20 rounded-3xl m-0">
                  🎉 No skill gaps detected! Strong match.
                </p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.missingSkills.map((sk, i) => (
                    <div
                      key={i}
                      className="flex flex-col bg-[var(--c-surface)] border border-red-500/20 shadow-[inset_0_4px_24px_rgba(239,68,68,0.03)] rounded-2xl p-5"
                    >
                      <div className="flex items-center justify-between mb-3 border-b border-[var(--c-border-2)] pb-3">
                        <span className="text-[15px] font-bold text-[var(--c-text)] truncate max-w-[60%]">
                          {sk.skill}
                        </span>
            ...
            <div className="flex flex-col animate-in fade-in zoom-in-95 duration-300">
              <div className="flex flex-col gap-4">
                {result.tailoringTips.map((tip, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-4 p-5 bg-[var(--c-surface)] border border-[var(--c-border)] shadow-sm rounded-2xl group hover:-translate-y-1 hover:border-[#f97316]/30 transition-all duration-300"
                  >
                    ...
                    <p className="text-[14px] md:text-[15px] leading-[1.7] text-[var(--c-text)] m-0 pt-0.5">
                      {tip}
                    </p>
                  </div>
                ))}
