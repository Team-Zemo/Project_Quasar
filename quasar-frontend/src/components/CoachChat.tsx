...
const SUGGESTIONS = [
  {
    ...
    text: "Find job opportunities matching my skills",
  },
  {
    icon: <Calendar size={18} className="text-orange-500" />,
    text: "Create a 2-week prep plan for FAANG interviews",
  },
  {
    icon: <Clock size={18} className="text-orange-500" />,
    text: "How to manage interview prep while working full-time?",
  },
  {
    icon: <LineChart size={18} className="text-orange-500" />,
    text: "Analyze my performance and give improvement tips",
  },
  {
    icon: <BookOpen size={18} className="text-orange-500" />,
    text: "Recommend courses to improve my weak areas",
  },
  {
    icon: <Lightbulb size={18} className="text-orange-500" />,
    text: "Best STAR method strategies for behavioral rounds",
  },
  {
    icon: <Map size={18} className="text-orange-500" />,
    text: "Create a 3-month career growth roadmap",
  },
  {
    icon: <Search size={18} className="text-orange-500" />,
    text: "Show me remote or internship job openings",
  },
];
  ...
  return (
    <div className="flex flex-col mx-auto w-full max-w-[860px] flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:var(--c-surface-3)_transparent] px-4 pt-6 pb-2 md:px-6">
        <AnimatePresence mode="wait">
          {isEmpty ? (
            ...
            >
              <div className="grid w-full max-w-[760px] grid-cols-1 gap-3 md:grid-cols-12">
                <div className="md:col-span-7 text-left md:pr-3">
                  <h1 className="mt-4 text-[44px] leading-[1.02] font-black tracking-tight text-[var(--c-text)] md:text-[64px]">
                    Quasar Coach
                  </h1>
                  <p className="mt-4 text-[14px] sm:text-[15px] text-[var(--c-text-dim)] max-w-[50ch] leading-relaxed">
                    Personalized interview prep, time management, and growth
                    planning in one focused conversation.
                  </p>
                </div>
                <div className="md:col-span-5 p-2">
                  <img
                    src="/ui/attending%20meet.svg"
                    alt="Career coaching visual"
              ...
              {/* Suggestion chips */}
              <div className="mt-5 text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--c-text-mute)]">
                Try one prompt
              </div>
              <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] max-sm:grid-cols-1 gap-3 mt-3 w-full max-w-[760px]">
                {SUGGESTIONS.map((s, i) => (
                  <motion.button
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <span className="text-[14px] text-[var(--c-text-dim)] leading-snug group-hover:text-[var(--c-text)] transition-colors">
                      {s.text}
                    </span>
                  </motion.button>
                ))}
            ...
            >
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold shrink-0 shadow-sm ${msg.role === "assistant" ? "bg-gradient-to-br from-orange-400 to-orange-600 text-white" : "bg-[var(--c-surface-3)] text-[var(--c-text)]"}`}
                  >
                    {msg.role === "assistant" ? (
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <span
                      className={`block text-[13px] font-bold mb-1.5 ${msg.role === "assistant" ? "text-orange-500" : "text-[var(--c-text-dim)]"}`}
                    >
                      {msg.role === "assistant"
                        ? "Quasar Coach"
                        : user?.name || "You"}
      ...
      <div className="border-t border-[var(--c-border)]  shrink-0 w-full px-4 py-3 md:px-6 md:py-4">
        <form
          className="flex items-end gap-3 bg-[var(--c-surface-2)] border border-[var(--c-border)] rounded-[18px] transition-colors duration-200 focus-within:border-orange-500/50 focus-within:ring-2 focus-within:ring-orange-500/10 shadow-sm p-1.5 px-3 md:p-2 md:px-3"
        ...
        >
          ...
          {streaming ? (
            <button
              type="button"
              className="w-10 h-10 mb-0.5 rounded-[12px] border-none flex items-center justify-center cursor-pointer transition-all duration-200 shrink-0 bg-red-500/10 text-red-500 hover:bg-red-500/20 active:scale-95"
              onClick={handleStop}
              title="Stop"
            >
        ...
        <p className="text-[11px] font-medium text-[var(--c-text-mute)] text-center mt-3">
          Quasar Coach is an AI assistant. Advice is AI-generated — use your
          best judgment.
        </p>
      </div>
    </div>
