...
export function JobPostingList({ onViewJob, onCreateJob, dummyJobs }: Props) {
  ...
  useEffect(() => {
    ...
    const url =
      filter === "all"
        ? "/api/recruiter/jobs"
        : `/api/recruiter/jobs?status=${filter}`;
    apiGet<JobPosting[]>(url)
      .then((res) => {
        if (res.success) setApiPostings(res.data);
      })
      .catch(() => {})
  ...
  const postings = dummyJobs
    ? filter === "all"
      ? dummyJobs
      : dummyJobs.filter((j) => j.status === filter)
    : apiPostings;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-[var(--c-text)] tracking-tight">
            Job Postings
          </h1>
          <p className="text-[var(--c-text-dim)] text-[14px] mt-1">
            {postings.length} total postings
          </p>
        </div>
        {/* ── Create button — tour target ───────────────────────────── */}
        <button
          id="tour-create-job-btn"
          onClick={onCreateJob}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)]">
          <Briefcase
            size={48}
            className="mx-auto text-[var(--c-text-mute)] mb-4"
          />
          <p className="text-[var(--c-text-dim)] text-[15px] font-semibold mb-2">
            No job postings yet
          </p>
          ...
          {postings.map((posting, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex items-center gap-3 sm:gap-5 p-3 sm:p-5 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-xl sm:rounded-2xl hover:border-[var(--c-accent)]/25 hover:bg-[var(--c-surface-2)] transition-all cursor-pointer group"
              >
                <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl bg-[var(--c-accent-dim)] flex items-center justify-center flex-shrink-0">
                  <Briefcase
                    size={18}
                    className="text-[var(--c-accent)] sm:hidden"
                  />
                  <Briefcase
                    size={20}
                    className="text-[var(--c-accent)] hidden sm:block"
                  />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
                    <h3 className="text-[13px] sm:text-[15px] font-bold text-[var(--c-text)] truncate max-w-[180px] sm:max-w-none">
                      {posting.title}
                    </h3>
                    <span
                ...
                <div className="flex items-center gap-3 sm:gap-6 flex-shrink-0">
                  ...
                  <Eye
                    size={14}
                    className="text-[var(--c-text-mute)] group-hover:text-[var(--c-accent)] transition-colors hidden sm:block"
                  />
                </div>
              </motion.div>
            );
