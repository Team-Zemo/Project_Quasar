...
import { CheckCircle2, AlertTriangle, Download, RotateCcw, Loader2, Sparkles, TrendingUp, AlertCircle } from 'lucide-react';
...
export function PostSessionResults({
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-[60vh]" style={{ padding: '32px' }}>
        <motion.div 
        ...
        >
          <div className="relative flex items-center justify-center w-20 h-20 mb-2">
            ...
            <Sparkles className="text-[var(--c-accent)] animate-pulse" size={28} />
          </div>
          <h3 className="text-[22px] font-black tracking-tight text-[var(--c-text)] m-0">Analyzing Performance</h3>
          <p className="text-[14px] text-[var(--c-text-dim)] mb-2">Our AI is reviewing your interview responses</p>
          <div className="flex flex-col gap-3 w-full max-w-[280px]">
            <div className="flex items-center gap-3 text-[13px] font-medium text-[var(--c-accent)] bg-[var(--c-accent-dim)] px-4 py-2 rounded-xl">
              <Loader2 className="animate-spin" size={16} /> Scoring STAR responses
            </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center w-full min-h-[60vh]" style={{ padding: '32px' }}>
        ...
        >
          ...
          <h3 className="text-[22px] font-black text-[var(--c-text)] m-0">Evaluation Unavailable</h3>
          <p className="text-[14px] text-[var(--c-text-dim)] leading-relaxed">{error || 'Could not evaluate this session. The transcript may be too short.'}</p>
          <div className="mt-4">
            <button onClick={onNewInterview} className="flex items-center gap-2 bg-[var(--c-surface-2)] text-[var(--c-text)] hover:bg-[var(--c-surface-3)] border border-[var(--c-border)] px-6 py-3 rounded-xl font-bold transition-all hover:scale-105 active:scale-95 shadow-sm">
  ...
  return (
    ...
    >
      {/* Overall Score Hero */}
      <motion.div variants={itemVariants} className="flex flex-col items-center justify-center p-8 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[24px] text-center shadow-[0_8px_32px_rgba(0,0,0,0.2)] w-full">
        <div className={`relative flex flex-col items-center justify-center w-[140px] h-[140px] rounded-full border-[6px] mb-5 shadow-inner transition-colors duration-500 ${evalData.passed ? 'border-green-500/80 bg-green-500/10 text-green-500' : 'border-orange-500/80 bg-orange-500/10 text-orange-500'}`}>
          <span className="text-[44px] font-black leading-none mt-2">{evalData.overallScore.toFixed(1)}</span>
          <span className="text-[12px] font-bold opacity-70 mt-1 uppercase tracking-wider">/ 10</span>
        </div>
        <div className={`inline-flex items-center px-5 py-2 rounded-full text-[13px] font-black tracking-widest mb-5 uppercase ${evalData.passed ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-orange-500/20 text-orange-400 border border-orange-500/30'}`}>
      ...
      {/* STAR Breakdown */}
      <motion.div variants={itemVariants} className="flex flex-col gap-5 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[20px] p-6 shadow-sm w-full">
        <h3 className="flex items-center gap-2 text-[18px] font-extrabold text-[var(--c-text)] m-0">
          <TrendingUp className="text-[var(--c-accent)]" size={20} /> STAR Breakdown
        </h3>
        <div className="flex flex-col gap-3.5">
          {starDimensions.map(dim => (
            <div key={dim.key} className="grid grid-cols-[130px_1fr_30px] items-center gap-4 max-sm:grid-cols-[100px_1fr_30px]">
              <span className="text-[13px] font-medium text-[var(--c-text-dim)] text-right">{dim.label}</span>
              ...
              <span className="text-[14px] font-bold text-[var(--c-text)]">{dim.score}</span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-[130px_1fr_30px] items-center gap-4 max-sm:grid-cols-[100px_1fr_30px]">
            <span className="text-[13px] font-medium text-[var(--c-text-dim)] text-right">Clarity</span>
            <div className="h-2.5 bg-[var(--c-surface-3)] rounded-full overflow-hidden w-full">
              <motion.div
                initial={{ width: 0 }}
            ...
            <span className="text-[14px] font-bold text-[var(--c-text)]">{evalData.clarityScore}</span>
          </div>
        </div>
      </motion.div>
      ...
      {evalData.categoryScores && Object.keys(evalData.categoryScores).length > 0 && (
        <motion.div variants={itemVariants} className="flex flex-col gap-5 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[20px] p-6 shadow-sm w-full">
          <h3 className="flex items-center gap-2 text-[18px] font-extrabold text-[var(--c-text)] m-0">
            <Sparkles className="text-[var(--c-purple)]" size={20} /> Skill Assessment
          </h3>
          <div className="flex flex-col gap-3.5">
            {Object.entries(evalData.categoryScores).map(([skill, score]) => (
              <div key={skill} className="grid grid-cols-[130px_1fr_30px] items-center gap-4 max-sm:grid-cols-[100px_1fr_30px]">
                <span className="text-[13px] font-medium text-[var(--c-text-dim)] text-right">{skillLabels[skill] || skill}</span>
                <div className="h-2.5 bg-[var(--c-surface-3)] rounded-full overflow-hidden w-full">
                  <motion.div
                    initial={{ width: 0 }}
                ...
                <span className="text-[14px] font-bold text-[var(--c-text)]">{score as number}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 max-md:grid-cols-1 gap-5 w-full">
        <div className="flex flex-col gap-4 p-6 rounded-[20px] border border-green-500/20 bg-green-500/5 shadow-sm">
          ...
          <ul className="flex flex-col gap-2.5 pl-0 m-0 list-none">
            {evalData.strengths?.map((s, i) => (
              <li key={i} className="flex items-start gap-2.5 text-[14px] text-[var(--c-text-dim)] leading-snug">
      ...
      {fillerBuckets.length > 0 && (
        <motion.div variants={itemVariants} className="flex flex-col gap-4 bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[20px] p-6 shadow-sm w-full">
          <SpeechHeatmap buckets={fillerBuckets} />
        </motion.div>
      )}

      {/* Actions */}
      <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-4 mt-4 w-full">
        <button
        ...
        <button onClick={onNewInterview} className="flex items-center gap-2 bg-[var(--c-surface-2)] text-[var(--c-text)] hover:bg-[var(--c-surface-3)] border border-[var(--c-border)] px-6 py-3.5 rounded-[14px] font-bold transition-all hover:scale-105 active:scale-95 shadow-sm">
          <RotateCcw size={18} />
          Start New Interview
        </button>
      </motion.div>
