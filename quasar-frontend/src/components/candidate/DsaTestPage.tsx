...
export function DsaTestPage({ appId, onComplete, onBack }: Props) {
  ...
  const handleLangChange = useCallback(
    (qId: string, lang: string) => {
      ...
      setCodeMap((prev) => {
        ...
        const wasStarter =
          prev[qId] === (q?.starterCode?.[oldLang] || "") || !prev[qId];
        if (wasStarter) {
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      const res = await apiPost<{
        ...
        passedCount: number;
        totalCount: number;
      }>(`/api/candidate/applications/${appId}/dsa/run`, {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await apiPost<{
        ...
        totalCount: number;
        score: number;
      }>(`/api/candidate/applications/${appId}/dsa/submit`, {
        questionId: question._id,
        language: langMap[question._id] || "javascript",
        code: codeMap[question._id] || "",
      });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[var(--bg-app)]">
        ...
        <Loader2
          size={36}
          className="text-[var(--c-accent)] animate-spin mb-4"
        />
        <p className="text-[var(--c-text-dim)] text-[14px] font-medium">
          Loading DSA Challenge...
        </p>
      </div>
    );
  }
  ...
  if (error && !testData) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[var(--bg-app)]">
        ...
        <p className="text-[var(--c-text)] text-[16px] font-bold mb-2">
          Cannot Start Test
        </p>
        <p className="text-[var(--c-text-dim)] text-[14px] max-w-md text-center">
          {error}
        </p>
        <button
          onClick={onBack}
          className="mt-6 flex items-center gap-2 px-4 py-2 rounded-xl text-[13px] font-bold text-[var(--c-text)] bg-[var(--c-surface-2)] hover:bg-[var(--c-surface-3)] transition-colors"
        >
  ...
  const qDiffColors =
    DIFFICULTY_COLORS[question.difficulty] || DIFFICULTY_COLORS.easy;
  const allowedLangs = LANG_OPTIONS.filter((l) =>
    testData.allowedLanguages.includes(l.key),
  );
    ...
    <div className="flex flex-col h-screen w-full bg-[#0e0e11] overflow-hidden">
      ...
      <div className="flex items-center justify-between h-[52px] px-4 bg-[#16161a] border-b border-[#2a2a33] shrink-0">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[var(--c-accent-dim)] border border-[var(--c-accent)]/25 text-[var(--c-accent)] text-[10px] font-bold uppercase tracking-widest rounded-full">
        ...
        <div className="flex items-center gap-3">
          {/* Timer */}
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-bold tracking-wide ${isTimeLow ? "bg-red-500/15 text-red-400 animate-pulse" : "bg-[#1f1f26] text-[#a1a1aa]"} border border-[#2a2a33]`}
          >
            ...
            {!showConfirmFinish ? (
              ...
              >
                ...
                <button
                  ...
                  className="px-3 py-1 rounded-md text-[12px] font-semibold text-[#a1a1aa] hover:text-white hover:bg-[#2a2a33] transition-colors"
                >
                  No
                </button>
                <button
                  ...
                  disabled={finishing}
                  className="flex items-center gap-1 px-3 py-1 rounded-md text-[12px] font-bold text-white bg-emerald-600 hover:bg-emerald-500 transition-colors disabled:opacity-50"
                >
                  {finishing ? (
          ...
          {testData.questions.map((q, i) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center justify-center w-[40px] h-[40px] rounded-xl text-[13px] font-bold transition-all border ${isActive ? "text-white border-[var(--c-accent)]/45 shadow-[0_0_16px_rgba(249,115,22,0.22)]" : "text-[#a1a1aa] border-transparent hover:border-[#2a2a33] hover:bg-[#1f1f26]"}`}
                style={
                  isActive
                    ? {
                        background:
          ...
          <div className="p-5 pb-3 border-b border-[#2a2a33]">
            <div className="flex items-center gap-2 mb-2">
              <span
                ...
                style={{
                  background: qDiffColors.bg,
                  color: qDiffColors.text,
                  border: `1px solid ${qDiffColors.border}`,
                }}
              >
                {question.difficulty}
              </span>
            ...
            <h2 className="text-[17px] font-bold text-[#e4e4e7] leading-tight">
              {question.title}
            </h2>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
            ...
            {question.constraints && (
              <div className="bg-[#1f1f26] rounded-xl p-3 border border-[#2a2a33]">
                ...
                <pre className="text-[12px] text-[#a1a1aa] whitespace-pre-wrap font-mono">
                  {question.constraints}
                </pre>
              </div>
            )}
            ...
            <div className="space-y-3">
              {question.sampleInput && (
                <div className="bg-[#1f1f26] rounded-xl p-3 border border-[#2a2a33]">
                  ...
                  <pre className="text-[12px] text-[#d4d4d8] whitespace-pre-wrap font-mono">
                    {question.sampleInput}
                  </pre>
                </div>
              )}
            ...
            {question.tags?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-2">
                {question.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2 py-0.5 rounded-md text-[10px] font-semibold text-[#71717a] bg-[#1f1f26] border border-[#2a2a33]"
                  >
                    {tag}
                  </span>
                ))}
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center gap-1 px-3 py-2 bg-[#16161a] border-b border-[#2a2a33] overflow-x-auto custom-scrollbar shrink-0">
            {allowedLangs.map((lang) => (
              <button
                ...
                className={`px-3 py-1.5 text-[12px] font-bold rounded-md transition-all whitespace-nowrap shrink-0 ${
          ...
          <div className="flex-1 min-h-0 bg-[#1e1e1e] relative">
            <Editor
              ...
              height="100%"
              language={
                LANG_OPTIONS.find((l) => l.key === currentLang)?.monacoLang ||
                "javascript"
              }
              value={currentCode}
              onChange={(val) =>
                setCodeMap((prev) => ({ ...prev, [question._id]: val ?? "" }))
              }
              theme="vs-dark"
              options={{
                fontSize: 14,
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[12px] font-bold text-[#a1a1aa]">
                      Test Results: {currentResult.passedCount}/
                      {currentResult.totalCount} passed
                    </span>
                    {currentResult.score !== undefined && (
                      <span
                        ...
                        style={{
                          color:
                            currentResult.score >= 60 ? "#22c55e" : "#ef4444",
                        }}
                      >
                  ...
                  <div className="space-y-2">
                    {currentResult.results.map((r, i) => (
                      <div
                        key={i}
                        className={`flex items-start gap-3 p-3 rounded-xl border ${r.passed ? "bg-emerald-500/5 border-emerald-500/15" : "bg-red-500/5 border-red-500/15"}`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-1">
                            ...
                            <span
                              className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${r.passed ? "bg-emerald-500/15 text-emerald-400" : "bg-red-500/15 text-red-400"}`}
                            >
                              {r.status}
                            </span>
                            {r.isHidden && (
                              <span className="text-[10px] font-semibold text-[#71717a] bg-[#1f1f26] px-1.5 py-0.5 rounded">
                                Hidden
                              </span>
                            )}
                          </div>
                          {!r.isHidden && (
                            <div className="grid grid-cols-3 gap-2 text-[11px] font-mono">
                              <div>
                                <p className="text-[10px] font-bold text-[#71717a] mb-0.5">
                                  Input
                                </p>
                                <pre className="text-[#a1a1aa] whitespace-pre-wrap break-all">
                                  {r.input || "(empty)"}
                                </pre>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <pre
                                  className={`whitespace-pre-wrap break-all ${r.passed ? "text-emerald-400" : "text-red-400"}`}
                                >
                                  {r.actual}
                                </pre>
                              </div>
                          ...
                          <div className="flex items-center gap-4 mt-1.5 text-[10px] text-[#71717a]">
                            <span className="flex items-center gap-1">
                              <Zap size={10} />
                              {r.time?.toFixed(0) || 0}ms
                            </span>
                            <span className="flex items-center gap-1">
                              <MemoryStick size={10} />
                              {(r.memory / 1024).toFixed(1) || 0}MB
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-[#16161a] border-t border-[#2a2a33] shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-bold text-white bg-gradient-to-r from-[var(--c-accent)] to-[#fdba74] hover:brightness-110 shadow-[0_4px_12px_rgba(249,115,22,0.28)] transition-all disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
              >
                {submitting ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
        ...
        {error && testData && (
          ...
          >
            ...
            <button
              ...
              className="ml-2 text-red-400 hover:text-white text-[12px] font-bold"
            >
              ✕
            </button>
          </motion.div>
