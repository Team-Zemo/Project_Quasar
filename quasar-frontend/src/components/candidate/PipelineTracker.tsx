...
import type {
  Application, JobPosting, PipelineConfig, TechRoundConfig,
...
function getStageStatus(stage: StageKey, app: Application): 'passed' | 'failed' | 'active' | 'upcoming' | 'skipped' {
  const s = app.status;
  switch (stage) {
    case 'screening':
      if (s === 'screening') return 'active';
      if (s === 'screening_failed') return 'failed';
      if (app.screeningResult) return app.screeningResult.passed ? 'passed' : 'failed';
      if (s === 'applied') return 'active';
      return 'passed';
    case 'mcq':
      if (['applied', 'screening', 'screening_failed'].includes(s)) return 'upcoming';
...
const statusColors: Record<string, { bg: string; border: string; text: string; ring: string }> = {
  passed: { bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.3)', text: 'var(--c-success)', ring: 'rgba(34,197,94,0.15)' },
  failed: { bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', text: 'var(--c-error)', ring: 'rgba(239,68,68,0.15)' },
  active: { bg: 'rgba(249,115,22,0.1)', border: 'rgba(249,115,22,0.4)', text: 'var(--c-accent)', ring: 'rgba(249,115,22,0.15)' },
  upcoming: { bg: 'var(--c-surface-2)', border: 'var(--c-border)', text: 'var(--c-text-mute)', ring: 'transparent' },
...
export function PipelineTracker({ appId, onBack, onStartMcq, onStartTechInterview, onStartHrInterview, refreshKey }: Props) {
    ...
    apiGet<Application>(`/api/candidate/applications`)
      .then(res => {
        if (res.success && Array.isArray(res.data)) {
          const found = (res.data as Application[]).find(a => a._id === appId);
          if (found) {
            setApp(found);
            const jobData = typeof found.jobPostingId === 'object' ? found.jobPostingId as JobPosting : null;
  ...
  if (!app) {
    return (
      <div className="max-w-3xl mx-auto py-8 px-4 text-center">
        <AlertCircle size={48} className="mx-auto text-[var(--c-text-mute)] mb-4" />
        <p className="text-[var(--c-text-dim)]">Application not found</p>
        <button onClick={onBack} className="mt-4 btn-ghost text-[13px]">← Back</button>
      </div>
    );
  }
  ...
  const buildJdContext = (): string => {
    ...
    return `You are interviewing a candidate for the position of "${job.title}" at "${job.company}".\n\n` +
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-black text-[var(--c-text)] tracking-tight">{job?.title || 'Application'}</h1>
        <p className="text-[13px] text-[var(--c-text-mute)] mt-1">{job?.company} {job?.location ? `• ${job.location}` : ''}</p>
      </div>
      ...
      <div className="relative">
        {visibleStages.map((stage, i) => {
          ...
          const StatusIcon = stageStatus === 'passed' ? CheckCircle : stageStatus === 'failed' ? XCircle : stageStatus === 'active' ? Loader2 : Clock;
            ...
            >
              ...
              {!isLast && (
                <div
                  ...
                  style={{
                    background: stageStatus === 'passed' || stageStatus === 'failed'
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: stageStatus === 'active' ? `0 0 24px ${colors.ring}` : 'none',
                }}
              >
                {/* Stage circle */}
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-[14px] font-bold" style={{ color: stageStatus === 'upcoming' ? 'var(--c-text-mute)' : 'var(--c-text)' }}>
                      {stage.label}
                      {stage.key === 'tech' && pipeline.techInterviewRounds && pipeline.techInterviewRounds.length > 1 && (
                        <span className="text-[11px] font-normal text-[var(--c-text-mute)] ml-1.5">
                          ({pipeline.techInterviewRounds.length} rounds)
                        </span>
                      )}
                    </h3>
                    <div className="flex items-center gap-1 text-[11px] font-semibold" style={{ color: colors.text }}>
                      <StatusIcon size={12} className={stageStatus === 'active' ? 'animate-spin' : ''} />
                  ...
                  {stage.key === 'screening' && app.screeningResult && (
                    <div className="text-[12px] text-[var(--c-text-dim)] space-y-1 mt-2">
                      <p>Match Score: <strong style={{ color: app.screeningResult.passed ? 'var(--c-success)' : 'var(--c-error)' }}>{app.screeningResult.matchScore}%</strong></p>
                      {app.screeningResult.matchedSkills.length > 0 && (
                        <p className="text-[11px]">Matched: {app.screeningResult.matchedSkills.slice(0, 5).join(', ')}</p>
                      )}
                      {app.screeningResult.summary && (
                        <p className="text-[11px] text-[var(--c-text-mute)] italic mt-1">{app.screeningResult.summary}</p>
                      )}
                    </div>
                  )}
                  ...
                  {stage.key === 'mcq' && app.mcqResult?.completedAt && (
                    <div className="text-[12px] text-[var(--c-text-dim)] mt-2">
                      <p>Score: <strong style={{ color: app.mcqResult.passed ? 'var(--c-success)' : 'var(--c-error)' }}>{app.mcqResult.percentage}%</strong> ({app.mcqResult.correctAnswers}/{app.mcqResult.totalQuestions} correct)</p>
                    </div>
                  )}
                  ...
                  {stage.key === 'tech' && app.techResults?.filter(r => r.score != null).length > 0 && (
                    <div className="flex items-center gap-3 mt-2">
                      {app.techResults.filter(r => r.score != null).map(r => (
                        <span key={r.roundNumber} className="text-[12px] px-2 py-0.5 rounded-lg" style={{
                          background: r.passed ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
                          color: r.passed ? 'var(--c-success)' : 'var(--c-error)',
                        }}>
                  ...
                  {stage.key === 'hr' && app.hrResult?.score != null && (
                    <div className="text-[12px] text-[var(--c-text-dim)] mt-2">
                      <p>Score: <strong style={{ color: app.hrResult.passed ? 'var(--c-success)' : 'var(--c-error)' }}>{app.hrResult.score?.toFixed(1)}/10</strong></p>
                    </div>
                  )}

                  {stage.key === 'result' && app.status === 'selected' && (
                    <div className="text-[12px] text-[var(--c-success)] font-semibold mt-2">
                      🎉 Congratulations! You've been selected. Total Score: {app.totalScore.toFixed(1)}
                  ...
                  {stage.key === 'tech' && canStartTech && currentTechConfig && (
                    <motion.button
                      initial={{ opacity: 0, scale: 0.95 }}
                      animate={{ opacity: 1, scale: 1 }}
                      disabled={actionLoading}
                      onClick={() => {
                        onStartTechInterview(appId, app.currentTechRoundNumber, currentTechConfig, buildJdContext(), app.status === 'tech_in_progress');
                      }}
                      className="flex items-center gap-2 mt-3 px-4 py-2 rounded-xl text-[13px] font-bold text-white bg-gradient-to-r from-blue-500 to-indigo-500 hover:brightness-110 transition-all shadow-md active:scale-95 disabled:opacity-50"
                    >
                      <Mic size={14} /> {app.status === 'tech_in_progress' ? 'Continue' : 'Start'} Tech Interview (Round {app.currentTechRoundNumber})
                    </motion.button>
                  )}
                  ...
                  {stage.key === 'hr' && canStartHr && (
                    <motion.button
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      disabled={actionLoading}
                      onClick={() => onStartHrInterview(appId, buildJdContext(), app.status === 'hr_in_progress')}
                      className="flex items-center gap-2 mt-3 px-4 py-2 rounded-xl text-[13px] font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:brightness-110 transition-all shadow-md active:scale-95 disabled:opacity-50"
                    >
                      <Users size={14} /> {app.status === 'hr_in_progress' ? 'Continue' : 'Start'} HR Interview
                    </motion.button>
                  )}
                </div>
                ...
                {/* Arrow */}
                {stageStatus === 'active' && (
                  <ChevronRight size={18} className="text-[var(--c-accent)] flex-shrink-0 mt-3" />
                )}
              </div>
            </motion.div>
          );
